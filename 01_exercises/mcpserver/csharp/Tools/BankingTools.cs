...
public class BankingTools
{
    ...
    {
        _logger.LogInformation("GetLoggedInUser called with TenantId={TenantId}, UserId={UserId}", tenantId, userId);
        
        try
        {
            var result = await _bankingService.GetUserAsync(tenantId, userId);
            _logger.LogInformation("GetLoggedInUser successful, returned user: {UserName}", result?.Name);
            return result;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "GetLoggedInUser failed with exception: {Message}", ex.Message);
            throw;
        }
    }
    ...
    {
        _logger.LogInformation("GetCurrentDateTime called");
        
        var now = DateTime.Now.ToUniversalTime();
        _logger.LogInformation("GetCurrentDateTime returning: {DateTime}", now);
        return now;
    }
    ...
    {
        _logger.LogInformation("SearchOffers called with accountType={AccountType}, requirement={Requirement}, tenantId={TenantId}", 
        ...
        try
        {
            ...
            _logger.LogInformation("Executing search for {AccountType} offers matching: {Requirement}", accountType, requirement);
            
            var offerTerms = await _bankingService.SearchOfferTermsAsync(tenantId, accType, requirement);
            
            _logger.LogInformation("SearchOffers completed - found {OfferCount} offers", offerTerms.Count);
